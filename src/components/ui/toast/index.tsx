...
function SuccessStatus({ message }: { message: string }) {
    return (
        <div className="fixed flex justify-center w-full top-[10%] z-[1000]">
            <div id="toast-success" className="border-solid border-[1px] border-[#0e9f6e40] flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
                ...
                <div className="ms-3 text-sm font-normal dark:text-white">Item moved successfully.</div>
            </div>
        </div>
    )
}
...
function WarningStatus({ message }: { message: string }) {
    return (
...
export default function ToastMessage({ typeMessage, message } : { typeMessage: TypeMessage, message: string }) {
    switch (typeMessage) {
